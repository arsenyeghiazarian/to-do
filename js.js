let id = 0;
            ...
            function toggleEdit(id) {
                ...
                if (el.firstElementChild.getAttribute('contenteditable') === 'false') {
                    ...
                    for(let i = 0; i < data.length; i++) {
                        if(data[i].id === id) {
                            data[i].title = el.firstElementChild.innerHTML
                            break;
                        }
                    }
                }
            ...
            function markAsDone(id) {
                for(let i = 0; i < data.length; i++) {
                    if(data[i].id === id) {
                        doneList.push(data.splice(i, 1)[0])
                        break;
                    }
                }
                getData();
            ...
            function undo(id) {
                for(let i = 0; i < doneList.length; i++) {
                    if(doneList[i].id === id) {
                        data.push(doneList.splice(i, 1)[0])
                        break;
                    }
                }
                getData();
            ...
            function getData() {
                ...
                if (!data.length) {
                    ...
                    data.sort((a, b) => a.id - b.id).forEach((el) => {
                    value += `
                            <li class="d-flex align-items-center list-row position-relative" id="${el.id}">
                                ...
                                <div class="btn-wrapper d-flex align-items-center">
                                    ...
                                    <button class="edit d-flex align-items-center justify-content-center ml-2" onclick="markAsDone(${el.id})">
                                        <svg class="bi bi-check" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                            <path fill-rule="evenodd" d="M13.854 3.646a.5.5 0 010 .708l-7 7a.5.5 0 01-.708 0l-3.5-3.5a.5.5 0 11.708-.708L6.5 10.293l6.646-6.647a.5.5 0 01.708 0z" clip-rule="evenodd"/>
                                        </svg>
                                    </button>
                                    <button class="edit d-flex align-items-center justify-content-center ml-2" onclick="remove(${el.id})">
            ...
            function getDoneData() {
                ...
                if (!doneList.length) {
                    ...
                    list.removeChild(list.childNodes[0])
                    doneList.sort((a, b) => a.id - b.id).forEach((el) => {
                    value += `
                            <li class="d-flex align-items-center list-row position-relative" id="${el.id}">
                                <p id="task" contenteditable="false">${el.title}</p>
                                <div class="btn-wrapper d-flex align-items-center">
                                    <button class="edit d-flex align-items-center justify-content-center ml-2" onclick="undo(${el.id})">
